...
## สถานะ
เปิดใช้ที่ https://cloth-wholesale-production.up.railway.app/ (ต้องล็อกอิน)
...
- ผู้จัดการแก้สินค้า/ลูกค้าจากหน้าหลักได้; ต้นทุนยังจัดการผ่าน Admin
- 1 ร้าน 1 สต๊อก ยังไม่มีหลายคลัง จัดซื้อ Supplier ขนส่ง การคืนสินค้าบางส่วน 
...
- ยังต้องทดลองกับเครื่องพิมพ์จริงก่อนใช้งานร้าน

## รุ่น 2 — ขายส่งและลูกหนี้
- หน้าขายมีตารางสี × ไซซ์: เลือกรุ่น กรอกจำนวน แล้วกดเพิ่มเข้าบิล สามารถผสมหลายรุ่นในบิลเดียวได้
- หน้า สินค้า → แก้ไข รองรับลิงก์รูป HTTPS (ยังไม่ใช่อัปโหลดไฟล์); หน้า ลูกค้า → แก้ไข ตั้งเครดิต 0–365 วัน
- สินค้า → ระดับราคา / ราคาลูกค้า: กำหนดระดับ 12/50 หรือจำนวนอื่นต่อ SKU โดยจำนวนคิดรวมทั้งรุ่น
- ลำดับราคา: ราคาเฉพาะลูกค้าแบบคงที่ → ระดับที่มีจำนวนขั้นต่ำสูงสุดที่ถึง → ราคาเดิม 1/6 ตัว โดยระดับที่สร้างทับ 1/6 จะใช้ราคาใหม่
- ราคาที่แก้ไขมีผลเฉพาะบิลใหม่ ใบเสร็จและรายการขายเดิมคงข้อมูลเดิม
- บิลใหม่กำหนดวันชำระตามเครดิตลูกค้า หรือเลือกวันเอง; ลูกค้าเดิมเครดิตเริ่มที่ 0 วัน (ครบกำหนดวันขาย)
- เมนูลูกหนี้แยกยอดยังไม่เกินกำหนด / 1–30 / 31–60 / มากกว่า 60 วัน / ยังไม่กำหนดวัน พร้อมสรุปรายลูกค้าและรับชำระผ่านหน้าบิลเดิม
- บิลเก่าจะไม่มีวันครบกำหนดอัตโนมัติ ผู้จัดการแก้วันจากหน้าบิลได้โดยระบุเหตุผล
- กลุ่มผู้จัดการได้รับสิทธิ์แก้สินค้า/ราคา/ลูกค้า; กลุ่มอื่นไม่เพิ่มสิทธิ์แก้ไขให้เอง
- ประวัติการแก้ไขจากหน้าใหม่ดูใน Admin → Audit events (อ่านอย่างเดียว); ไม่ครอบคลุมการแก้ข้อมูลโดยตรงผ่าน Admin เดิม
- Migration 0002 เพิ่มฟิลด์และตาราง ไม่ลบหรือคำนวณยอดบิลเดิมใหม่

การตรวจรอบนี้: Django tests ครอบคลุมราคาแบบคละไซซ์ ราคาเฉพาะลูกค้า เครดิต การแก้ข้อมูล สิทธิ์ อายุหนี้ และการคงข้อมูลบิลเดิม; GitHub Actions ใช้ PostgreSQL รวมการขายพร้อมกัน

## รุ่น 3 — รวมใบเสร็จและรายงานรับเงิน
- เมนู ใบเสร็จ / รวมใบเสร็จ ค้นหาตามลูกค้า วันที่รับเงิน เลข RC/SO หรือชื่อลูกค้า
- เลือก 2–100 ใบในหน้าเดียวกันเพื่อสร้างเอกสาร RS ใบสรุปรวมใบเสร็จ พร้อมพิมพ์ A4 / 80 มม. / Save as PDF
- รวมเฉพาะลูกค้าที่ระบุตัวตนรายเดียวกัน ข้อมูลร้าน/ผู้ซื้อในต้นฉบับต้องตรงกัน ไม่รวมใบเสร็จยกเลิก
- RS เป็นเอกสารสรุป ไม่ใช่ใบเสร็จรับเงินใหม่ ไม่แทนที่หรือลบ RC เดิม ไม่บันทึกรับเงินและไม่ตัดสต๊อกซ้ำ
- รับเงินหลายงวดของบิลเดียวกันจะแสดงสินค้าหนึ่งครั้งต่อบิล ยอดสรุปเท่ากับยอดรับเงินจริงของ RC ที่เลือก ไม่ใช่ยอดเต็มบิล
- เก็บภาพข้อมูลเอกสาร ณ วันที่รวม; หากยกเลิก RC ต้นฉบับภายหลัง RS จะแสดงคำเตือนแม้ตอนพิมพ์
- ยกเลิก RS ได้พร้อมเหตุผลโดยไม่กระทบ RC เดิม; สามารถทำใบสรุปใหม่ได้ เอกสารสรุปซ้ำไม่ถูกนำไปนับรายงานรับเงิน
- รายงานรับเงินรายวันแยก เงินสด/โอน/QR/บัตร นับจาก RC ที่ยังไม่ยกเลิก ตามวันรับเงินเวลาไทย เลือกช่วงไม่เกิน 366 วัน
- รายงานเป็นยอดตามการบันทึก ไม่ใช่กระทบยอดธนาคารหรือบัญชีคืนเงิน และไม่ใช่ยอดขาย/กำไร
